  ...
  for (var i = 0; i < selectedElements.length; ++i) {
    ...
    if (selectedElement.isPartial()) {
      ...
      var newt =
        ...
        txt2add +
        t.substr(selectedElement.getEndOffsetInclusive() + 1);
      text.setText(newt);
      if (txt2add.length) {
      ...
      var t = ele.getType();
      if (
        t == DocumentApp.ElementType.TABLE_CELL ||
        ...
        if (i == selectedElements.length - 1)
          doc.setCursor(doc.newPosition(ele, txt.length - 1));
      } else {
        if (
          t == DocumentApp.ElementType.TEXT ||
          t == DocumentApp.ElementType.PARAGRAPH
        )
    ...
    if (selectedElement.isPartial()) {
      ...
      if (surfix.length > 0) {
        ...
        se =
          se == -1
            ? ss + surfix.length
            : selectedElement.getEndOffsetInclusive() + 2 + surfix.length;
...
function zeropading(x, size) {
  return x.toString().replace(/(\D?)(\d+)/g, function (m, p1, p2, offset, s) {
    return p1 + zerofill(p2, size);
  });
}
...
function caseConvert(t, flag) {
  switch (flag) {
    ...
    case 2:
      ...
      break;
    case 3:
      return t.replace(/([a-zA-Z]+)/g, function (m, p1, s) {
  ...
  toLunar: function (objDate) {
    ...
    for (i = 1; i < 13 && offset > 0; i++) {
      ...
      if (leap > 0 && i == leap + 1 && lunarDate.isLeap == false) {
        ...
        temp = this.leapDays(lunarDate.year);
      } else {
...
function getWeekMonday(d) {
  ...
  return (
    1 +
    Math.floor(
      ((d - onejan) / 86400000 - (((onejan.getDay() + 6) % 7) + 1)) / 7
    )
  );
}
  ...
  while (i < format.length) {
    if (
      ...
      switch (format[i + 3]) {
        ...
        default:
          result =
            result + format[i] + format[i + 1] + format[i + 2] + format[i + 3];
      }
      i += 3;
    } else if (
      format[i] === '%' &&
